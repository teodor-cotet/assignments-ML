...
class SVMTrainer(object):
    ...
    def _compute_multipliers(self, X, y):
        ...
        # P = cvxopt.matrix(...)
        P = cvxopt.matrix(np.multiply(np.outer(y, y.T), K))

        # TODO calculeaza valoarea vectorului q
        # q = cvxopt.matrix(...)
        q = cvxopt.matrix(-1 * np.ones((n_samples, 1)))
        ...
        # G_std = cvxopt.matrix(...)
        # h_std = cvxopt.matrix(...)
        G_std = cvxopt.matrix(cvxopt.spdiag(cvxopt.matrix(-1, (n_samples, 1))))
        h_std = cvxopt.matrix(np.zeros((n_samples, 1)))

        # TODO seteaza G_slack si h_slack pentru a rezolva a <= C
        # G_slack = cvxopt.matrix(...)
        # h_slack = cvxopt.matrix(...)
        G_slack = cvxopt.matrix(cvxopt.spdiag(cvxopt.matrix(1, (n_samples, 1))))
        h_slack = cvxopt.matrix(self._c, (n_samples, 1))
        ...
        # b = cvxopt.matrix(...)
        A = cvxopt.matrix(np.transpose(y), (1, n_samples), tc = 'd')
