...
if __name__ == "__main__":
    ...
    for row_no in range(rows_no):
        ...
        for col_no in range(cols_no):
            idx = np.random.randint(data["train_imgs"].shape[0]) # take random img from train
            labels[(row_no, col_no)] = data["train_labels"][idx]
            row = np.hstack((row, data["train_imgs"][idx].reshape(28, 28))) # reshape because is a line vector
        full_img = np.vstack((full_img, row)) # concaternate them to from the bigger image
