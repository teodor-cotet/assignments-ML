...
class Neuron:
    ...
    def __init__(self, inputs_no, transfer_function):
        ...
        self.weights = np.random.normal(
            0,
            np.sqrt(2.0 / float(self.inputs_no)),
            self.inputs_no
        )
        ...
        # Computed values
        self.a = 0
        # value after applying f to it
        self.output = 0

        # Gradients
        self.g_weights = np.zeros((self.inputs_no))
        self.g_bias = 0
    ...
    def forward(self, inputs):
        ...
        self.a = 0
        for i in range(self.inputs_no):
            self.a += self.weights[i] * inputs[i][0] + self.bias
        #self.a = np.matmul(self.weights, inputs) + self.biases
        self.output = self.f(self.a)
        return self.output

    def backward(self, inputs, output_neuron, a):
        # output_neuron raw
        self.g_bias = 0 #output_neuron

        # TODO (2.b.ii)
        # Compute the gradients w.r.t. the weights (self.g_weights)
        #self.g_weights =  inputs.dot(self.g_biases.T).T
        for i in range(self.inputs_no):
            #self.g_weights[i] = inputs[i] * self.f(self.a, True)
            self.g_weights[i] = inputs[i] * output_neuron * self.f(a, True)
    ...
    def update_parameters(self, learning_rate):
        self.bias = self.bias - learning_rate * self.g_bias
        for i in range(self.inputs_no):
            self.weights[i] = self.weights[i] - learning_rate * self.g_weights[i]
